# JIT Ablation Bench Implementation Plan

> **For Claude:** Execute this plan using the skill chosen during Execution Handoff (see end of plan).
> Planning dir: .planning/

**Goal:** Build `ablation_bench` binary that identifies JIT-GOOD/BAD/NEUTRAL contracts via per-tx ablation with Welch's t-test.

**Architecture:** New binary at `examples/runner/src/bin/ablation_bench.rs` reusing `bin_common.rs` (BinLoader, AOT cache, JitHandler). Adds snapshot collection, ablation loop, and zero-dependency statistical tests. Single file ~500 lines.

**Tech Stack:** Rust, clap, revm CacheDB, revmc JIT, bin_common.rs infra

**Worktree:** `.worktrees/ablation` (branch `feature/ablation-bench`)
...
Append after the existing `frame_bench` bin entry in `examples/runner/Cargo.toml`:

```toml
[[bin]]
name = "ablation_bench"
path = "src/bin/ablation_bench.rs"
```

**Step 2: Create empty placeholder**

Create `examples/runner/src/bin/ablation_bench.rs`:

```rust
fn main() {
    println!("ablation_bench placeholder");
}
```

**Step 3: Verify it compiles**

Run: `cargo check -p revmc-examples-runner --bin ablation_bench`
Expected: compiles clean

**Step 4: Commit**

```bash
git add examples/runner/Cargo.toml examples/runner/src/bin/ablation_bench.rs
git commit -m "feat(ablation): register ablation_bench binary"
```
...
## Task 2: Implement CLI args and scaffold phases

**Files:**
- Modify: `examples/runner/src/bin/ablation_bench.rs`

**Step 1: Write CLI + phase scaffold**

Replace placeholder with full scaffold:
...
use bin_common::{
    build_op_cfg, build_op_tx, compile_all_contracts_with_cache, extract_gas,
...
fn main() {
    ...
    eprintln!("  {} txs, {} accounts, {} codes", loader.tx_count(), loader.account_count(), loader.code_count());

    // Phase 1: Compile contracts
    eprintln!("\n=== Phase 1: Load JIT cache ===");
    let all_codes: HashMap<B256, _> = loader.code_values().iter().map(|(h, b)| (*h, b.clone())).collect();
    let compiled = compile_all_contracts_with_cache(&all_codes, OptimizationLevel::Aggressive, Path::new(&args.cache_dir));
...
```bash
git add examples/runner/src/bin/ablation_bench.rs
git commit -m "feat(ablation): CLI args and phase scaffold"
```

---

## Task 3: Implement per-tx snapshot collection

**Files:**
- Modify: `examples/runner/src/bin/ablation_bench.rs`

**Step 1: Implement `collect_snapshots`**

Replace the `collect_snapshots` todo with:

```rust
/// Run the block sequentially, capturing CacheDB state before each non-deposit tx.
/// Returns Vec<(tx_index, snapshot_db)>.
fn collect_snapshots(loader: &BinLoader) -> Vec<(usize, CacheDB<EmptyDB>)> {
    ...
    for (i, tx_bin) in loader.raw_txs().iter().enumerate() {
        // Skip deposit txs (type 0x7e)
        if tx_bin.tx_type == 0x7e {
            continue;
        }
        // Clone DB state BEFORE executing this tx
        let snapshot_db = evm.0.ctx.db().clone();
        snapshots.push((i, snapshot_db));

        // Execute tx to advance state
...
```bash
git add examples/runner/src/bin/ablation_bench.rs
git commit -m "feat(ablation): per-tx snapshot collection via CacheDB clone"
```

---

## Task 4: Implement ablation loop with contract discovery

**Files:**
- Modify: `examples/runner/src/bin/ablation_bench.rs`

**Step 1: Add data structures and replay helper**

Add these types and functions after the `collect_snapshots` function:
...
/// Replay a single tx from a snapshot, return execution time.
/// `functions` controls which contracts use JIT (empty = all native).
fn replay_tx(
    tx_bin: &TxBin,
...
/// Discover which JIT-compiled contracts are touched by replaying the tx once
/// with an instrumented handler that tracks dispatched bytecode hashes.
fn discover_contracts(
    tx_bin: &TxBin,
    snapshot_db: &CacheDB<EmptyDB>,
    chain_id: Option<u64>,
    all_functions: &HashMap<B256, RawEvmCompilerFn>,
) -> HashMap<B256, usize> {
    use revm::handler::{EvmTr, FrameResult, ItemOrResult};
    use bin_common::{run_jit_or_native, BenchError};
    ...
    // Track which hashes are JIT-dispatched
    let mut counts: HashMap<B256, usize> = HashMap::new();

    // Manual exec loop that records dispatched contracts
    let first_frame_input = match evm.set_first_frame_return() {
        Ok(ItemOrResult::Item(init)) => init,
        _ => return counts,
    };
    ...
    loop {
        // Check current frame's bytecode hash
        let frame = evm.0.frame_stack.get();
        let hash = frame.interpreter.bytecode.get_or_calculate_hash();
        if all_functions.contains_key(&hash) {
            *counts.entry(hash).or_insert(0) += 1;
        }

        let call_or_result = match run_jit_or_native(&mut evm, all_functions) {
            Ok(r) => r,
            Err(_) => break,
        };
...
**Step 2: Implement the ablation loop in main**

Replace the Phase 3 TODO in `main()` with:

```rust
    // Phase 3: Run ablation
    eprintln!("\n=== Phase 3: Per-tx ablation ===");
    let chain_id = loader.raw_txs().first().and_then(|tx| tx.chain_id);
    let all_fns_map: HashMap<B256, RawEvmCompilerFn> = all_functions.iter().map(|(&h, &f)| (h, f)).collect();
    let empty_fns: HashMap<B256, RawEvmCompilerFn> = HashMap::new();

    let mut tx_results: Vec<TxAblationResult> = Vec::new();

    for (snap_idx, (tx_index, snapshot_db)) in snapshots.iter().enumerate() {
        let tx_bin = &loader.raw_txs()[*tx_index];

        // Discover contracts touched by this tx
        let contract_counts = discover_contracts(tx_bin, snapshot_db, chain_id, &all_fns_map);
...
Run: `cargo check -p revmc-examples-runner --bin ablation_bench`
Expected: compiles (Phase 4 still TODO)

**Step 4: Commit**

```bash
git add examples/runner/src/bin/ablation_bench.rs
git commit -m "feat(ablation): per-tx ablation loop with contract discovery"
```

---

## Task 5: Implement Welch's t-test and reporting

**Files:**
- Modify: `examples/runner/src/bin/ablation_bench.rs`

**Step 1: Add statistics module**

Add these functions (ported from hybrid-dispatch `ablation_analysis.rs`):
...
fn regularized_incomplete_beta(a: f64, b: f64, x: f64) -> f64 {
    ...
    let log_prefix = a * x.ln() + b * (1.0 - x).ln()
        - ln_gamma(a) - ln_gamma(b) + ln_gamma(a + b) - a.ln();
    ...
    for m in 1..=200 {
        ...
        d = 1.0 + num_even * d; if d.abs() < TINY { d = TINY; } c = 1.0 + num_even / c; if c.abs() < TINY { c = TINY; }
        d = 1.0 / d; result *= c * d;
        let num_odd = -(a + m_f) * (a + b + m_f) * x / ((a + 2.0 * m_f) * (a + 2.0 * m_f + 1.0));
        d = 1.0 + num_odd * d; if d.abs() < TINY { d = TINY; } c = 1.0 + num_odd / c; if c.abs() < TINY { c = TINY; }
        d = 1.0 / d; let delta = c * d; result *= delta;
...
fn welch_t_test(a: &[f64], b: &[f64]) -> Option<WelchResult> {
    ...
    if se_sq == 0.0 {
        ...
        return Some(WelchResult { mean_diff: md, p_value: if md == 0.0 { 1.0 } else { 0.0 } });
    }
    let t_stat = (mean_a - mean_b) / se_sq.sqrt();
    let df = se_sq.powi(2) / (var_a.powi(2) / (n_a_f.powi(2) * (n_a_f - 1.0))
        + var_b.powi(2) / (n_b_f.powi(2) * (n_b_f - 1.0)));
    let p_value = 2.0 * (1.0 - t_cdf(t_stat.abs(), df));
    Some(WelchResult { mean_diff: mean_a - mean_b, p_value })
}
...
fn cohen_d(a: &[f64], b: &[f64]) -> f64 {
    ...
    if pooled_sd == 0.0 { 0.0 } else { (mean_a - mean_b) / pooled_sd }
}

fn median(v: &[Duration]) -> f64 {
...
fn verdict(p_value: f64, mean_diff: f64, alpha: f64) -> &'static str {
    if p_value >= alpha { "NEUTRAL" }
    else if mean_diff > 0.0 { "JIT-GOOD" }
    else { "JIT-BAD" }
}
```

**Step 2: Add reporting in Phase 4**

Replace the Phase 4 TODO with:

```rust
    // Phase 4: Aggregate and report
    eprintln!("\n=== Phase 4: Block summary ===\n");

    // Per-contract aggregation across all txs
    struct AggEntry {
        appearances: usize,
        total_frames: usize,
        deltas_us: Vec<f64>,  // all per-tx mean_diffs for this contract
    }
    let mut agg: HashMap<B256, AggEntry> = HashMap::new();

    // Per-tx report (verbose)
    for txr in &tx_results {
        let jit_med = median(&txr.jit_times);
        let native_med = median(&txr.native_times);
        ...
        for cr in &txr.contract_results {
            ...
            let ablat_med = median(&cr.ablated_times);
    ...
    entries.sort_by(|a, b| {
        b.1.deltas_us.iter().sum::<f64>().partial_cmp(&a.1.deltas_us.iter().sum::<f64>()).unwrap()
    });
    ...
    for (hash, entry) in &entries {
        ...
        // One-sample t-test on deltas (H0: mean delta = 0)
        ...
        let (agg_p, v) = if entry.deltas_us.len() >= 2 {
            ...
            // Single appearance: use per-tx result directly
            let single_p = if entry.deltas_us[0].abs() > 1.0 { 0.01 } else { 1.0 };
            (single_p, verdict(single_p, entry.deltas_us[0], args.alpha))
        };

        match v {
            "JIT-GOOD" => { n_good += 1; good_contracts.push(**hash); }
            "JIT-BAD" => n_bad += 1,
            _ => n_neutral += 1,
        }
...
```bash
git add examples/runner/src/bin/ablation_bench.rs
git commit -m "feat(ablation): Welch t-test statistics and two-level reporting"
```
...
Run: `cargo build -p revmc-examples-runner --bin ablation_bench --release`
...
```bash
cargo run -p revmc-examples-runner --bin ablation_bench --release -- \
  --block 38004930 --cache-dir /tmp/jit_cache --rounds 5 --warmup 1 --verbose
```

Expected:
- Phase 0-4 complete without panic
- Per-tx ablation details printed (verbose)
- Block summary with JIT-GOOD/BAD/NEUTRAL counts
- Gas values consistent (no mismatches)
...
```bash
cargo run -p revmc-examples-runner --bin ablation_bench --release -- \
  --block 38004930 --cache-dir /tmp/jit_cache --rounds 15 --warmup 3 --verbose
```
...
```bash
git add -u
git commit -m "fix(ablation): smoke test fixes"
```
...
- **Group A** (serial): Task 1, Task 2 (scaffold must come first)
- **Group B** (parallel, after A): Task 3, Task 5 (snapshot collection and statistics are independent modules)
- **Group C** (after B): Task 4 (ablation loop depends on both snapshot + stats)
- **Group D** (after C): Task 6 (integration test)

**Parallelism score:** 2/6 tasks can run in parallel (Group B). Most work is sequential due to code being in a single file.

---

### Execution Handoff

This plan has 6 tasks, low parallelism (single-file changes), moderate complexity. The tasks are sequential in nature since they build on each other in one file.
